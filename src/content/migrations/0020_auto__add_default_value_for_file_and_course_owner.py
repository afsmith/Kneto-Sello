...
class Migration(SchemaMigration):
    ...
    depends_on = (('management', '0001_initial'),)

    def forwards(self, orm):
        users = User.objects.filter(userprofile__role=UserProfile.ROLE_SUPERADMIN).order_by('id')

        if len(users) > 0:
            File.objects.filter(owner__isnull=True).update(owner=users[0])
            Course.objects.filter(owner__isnull=True).update(owner=users[0])
    ...
    models = {
        ...
        'content.file': {
            ...
            'key': ('django.db.models.fields.CharField', [], {'default': "'xEjMC7DU64OSlMizEnptc2'", 'unique': 'True', 'max_length': '22'}),
            ...
            'subkey_conv': ('django.db.models.fields.CharField', [], {'default': "'xmB7S'", 'max_length': '5'}),
            'subkey_orig': ('django.db.models.fields.CharField', [], {'default': "'SFqzt'", 'max_length': '5'}),
            'title': ('django.db.models.fields.CharField', [], {'max_length': '150'}),
